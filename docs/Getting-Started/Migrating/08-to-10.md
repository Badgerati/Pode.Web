...
In principle, all of the parameters on `Set-PodeWebHomePage` should map over 1-to-1 on `Add-PodeWebPage`, but you'll need to additional supply `-Name 'Home' -Path '/' -HomePage` to `Add-PodeWebPage` as well.
